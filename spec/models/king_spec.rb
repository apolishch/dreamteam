...
RSpec.describe King, type: :model do
    ...
    describe '#pieces_causing_check' do
        let(:game) {FactoryBot.create(:game)}
        let!(:queen_b) {FactoryBot.create(:queen, game_id: game.id, x_position: 2, y_position: 3, color: false)}
        ...
        xit 'builds an array with only the pieces that cause a king to be in check' do
     ...
     describe '#in_check?' do
        ...
        xit 'returns true if the user\'s king is in check' do
            expect(king_b.in_check?).to eq(true)
        end
        
        xit 'returns false if a user\'s king is not in check' do
